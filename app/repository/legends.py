...
def create(session: Session, legend: LegendCreate, image_url: str, user_id: int) -> Legend:
...
def get_one(session: Session, legend_id: int) -> Optional[Legend]:
    return session.get(Legend, legend_id)


def update_one( legend_id: int, legend: LegendUpdate, session: Session) -> Optional[Legend]:
...
def delete_one(session: Session, legend_id: int) -> bool:
    db_legend = session.get(Legend, legend_id)
    if db_legend:
...
def get_all_cantons(session: Session):
